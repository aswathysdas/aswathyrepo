		  ...
		  <script>
			...
			// });
			   		...
			   		'success':function(data){
			   			...
			   			$('#vv').empty();
							...
							for (var i = 0; i < dat.length; i++) {
								...
								toAppend+="<div class='grid_1_of_4 images_1_of_4 products-info'><img style='width:200px;height:280px;' src='<?php echo base_url(); ?>uploads/"+dat[i].vchr_product_img+"'> <a  onClick='prodetails("+dat[i].pk_int_prdct_id+")'>"+dat[i].vchr_prdct_nm+"</a><h3>Rs."+dat[i].int_sell_price+"</h3><ul><li><button type='button' class='btn btn-info' value='"+dat[i].pk_int_prdct_id+"' onClick='prodbuy("+dat[i].pk_int_prdct_id+",\""+dat[i].int_sell_price+"\",\""+dat[i].int_quantity+"\")'>Buy Now</button></li></ul></div>";
							};
							$('#vv').append(toAppend);
			   		}
			   	});
			  ...
			  function prodetails(pk){
			  	...
			  	$.ajax({
			   		'type':"POST",
			   		'url' :"<?php echo base_url(); ?>"+"index.php/Loginctrl/prodonclick",
			   		'datatype':"json",
			   		'data' :{name:pk},
			   		'success':function(data){
	...
	<body id="dtsp">
		...
		<div class="header">
			...
			<div class="header-top-nav">
				<ul>
					<li id="lg"><a href="logout">Logout</a></li>
				</ul>
			</div>
			<div class="clear"> </div>
		</div>
		...
		<div class="top-nav">
			...
			<ul>
				...
				<li><a href="">Customer</a>
					<ul>
						<li><a href="../regstrctrl">Add Customer</a></li>
						<li><a href="../Category_ctrl/viewcust">View Customer</a></li>
						<li><a href="../Category_ctrl/suspendcust">Suspend Customer</a></li>
					</ul>
			</ul>
		</div>
		</div>
