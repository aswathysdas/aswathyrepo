		...
		<script src="<?php echo base_url('js/expandcollapsed.js');?>" type="text/javascript"></script>
		  <script>
		    // You can also use "$(window).load(function() {"
			    ...
			    function prodpurch(lgid){
			    	alert(lgid);
			    	$.ajax({
			   		'type':"POST",
			   		'url' :"http://api.baabtra.com/Mobilestore_service/index.php/Category_ctrl/proooducts",
			   		'datatype':"json",
			   		'data' :{name:lgid},
			   		'success':function(data){
			   			...
			   			//$('#c').empty();
							...
							for (var i = 0; i < dat.length; i++) {
								...
								toAppend+="<div class='row'><div class='col-sm-offset-3 col-sm-8'><div class='panel panel-default'><div class='panel panel-heading'><h4>Purchase Details</h4></div><div class='panel panel-body'><table class='table table-striped'><tr><th>Sl No</th><th>Product Name</th><th>Price</th><th>Quantity</th><th>Date</th></tr><tr><td>"+dat[i].pk_int_purchase_id+"</td><td>"+dat[i].vchr_prdct_nm+"</td><td>"+dat[i].int_total_amount+"</td><td>"+dat[i].int_quantity+"</td><td>"+dat[i].dat_date+"</td></tr></table></div></div></div></div>";
							};
							$('#seepro').append(toAppend);
			   		}
			   	});
					  				...
					  				<div class="panel-body">
					  					<table id="ct" class="table table-striped">
					  						...
					  						foreach ($customer as $row) {
					  							echo "<tr>";
					  							echo "<td>".++$c."</td><td>".$row->vchr_fname."</td><td>".$row->vchr_lname."</td><td>".$row->vchr_address."</td><td>".$row->int_pincode."</td><td>".$row->vchr_mobilenum."</td><td>".$row->dat_date."</td><td><button type='button' class='btn btn-link' value='".$row->fk_int_login_id."' onClick='prodpurch(this.value)'>Purchase Details</button></td>";
					  							echo "</tr>";
					  						}
					  						?>
					  					</table>
